  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/admin/teachers', {
        ...
        body: JSON.stringify({ name, phone, email: email.trim().toLowerCase(), specializations }),
      });
      ...
      <div className="bg-white w-full max-w-md rounded-2xl shadow-xl border border-slate-200 p-6 space-y-4">
        <div className="flex justify-between items-center border-b border-slate-100 pb-3">
          <div>
            ...
            <p className="text-xs text-slate-500">Cấp email đăng nhập Cổng Điểm Danh riêng</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-3 text-xs">
          ...
          <div>
            ...
            <span className="text-[10px] text-indigo-600 mt-0.5 inline-block">
              Email này dùng làm tài khoản đăng nhập trực tiếp.
            </span>
          </div>
